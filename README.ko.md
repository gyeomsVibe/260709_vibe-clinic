...
```bash
npx -y --package=vibe-diagnosis vibe-diag init                        # .vibe-diagnosis/ 초기화 + MCP 자동 설정
npx -y --package=vibe-diagnosis vibe-diag run                         # 모든 진단 실행
npx -y --package=vibe-diagnosis vibe-diag run --json                  # JSON 출력 (CI/CD용)
npx -y --package=vibe-diagnosis vibe-diag dashboard                   # 웹 대시보드 열기
npx -y --package=vibe-diagnosis vibe-diag config get                  # BYOK 설정 확인
npx -y --package=vibe-diagnosis vibe-diag config set provider openai  # AI 프로바이더 설정
npx -y --package=vibe-diagnosis vibe-diag config set apiKey sk-...    # API 키 설정
npx -y --package=vibe-diagnosis vibe-diag config set model gpt-4o     # 모델명 설정
npx -y --package=vibe-diagnosis vibe-diag repair <diagId>             # 특정 진단 AI 자동 수리
npx -y --package=vibe-diagnosis vibe-diag repair --all                # 실패한 모든 진단 자동 수리
