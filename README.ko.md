...
> [!IMPORTANT]
> npm/npx 레지스트리 패키지 기반 실행은 현재 지원되지 않으며 표준이 아닙니다. 레지스트리 버전은 최신 저장소 상태와 일치하지 않으므로, 반드시 로컬 저장소 루트에서 `node ./bin/vibe-diag.js` (Windows는 `node .\bin\vibe-diag.js`)를 사용하여 직접 실행해야 합니다.

```bash
# Windows PowerShell
node .\bin\vibe-diag.js init                        # .vibe-diagnosis/ 초기화 + MCP 자동 설정
node .\bin\vibe-diag.js run                         # 모든 진단 실행
node .\bin\vibe-diag.js run --json                  # JSON 출력 (CI/CD용)
node .\bin\vibe-diag.js dashboard                   # 웹 대시보드 열기
node .\bin\vibe-diag.js config get                  # BYOK 설정 확인
node .\bin\vibe-diag.js config set provider openai  # AI 프로바이더 설정
node .\bin\vibe-diag.js config set apiKey sk-...    # API 키 설정
node .\bin\vibe-diag.js config set model gpt-4o     # 모델명 설정
node .\bin\vibe-diag.js repair <diagId>             # 특정 진단 AI 자동 수리
node .\bin\vibe-diag.js repair --all                # 실패한 모든 진단 자동 수리
...
> [!TIP]
> **대시보드 작동 검증 완료:** Windows PowerShell 환경에서 `node .\bin\vibe-diag.js dashboard` 실행 및 `http://localhost:7700`을 통한 API 호출 작동이 검증 완료되었습니다.
...
2. `vibe-diagnosis-vscode-1.1.3.vsix` 선택
