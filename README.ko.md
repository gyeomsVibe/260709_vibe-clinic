...
> [!IMPORTANT]
> npm/npx 레지스트리 패키지 기반 실행은 현재 지원되지 않으며 표준이 아닙니다. 레지스트리 버전은 최신 저장소 상태와 일치하지 않으므로, 반드시 로컬 저장소 루트에서 `node ./backend/bin/vibe-clinic.js` (Windows는 `node .\bin\vibe-clinic.js`)를 사용하여 직접 실행해야 합니다.
