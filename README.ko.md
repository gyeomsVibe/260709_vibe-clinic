...
| 영역 | 책임 |
| --- | --- |
| `backend/` | 실행 엔진, CLI, MCP, 테스트 |
| `frontend/` | 정식 V2 UI와 보관 V1 UI |
| `shared/` | API 계약 단일 진실원 |
| `docs/` | 기획, handoff, 운영 자료, 비런타임 이미지 |
| `integrations/` | VS Code 확장과 Claude Vibe Check 통합 |

모든 AI 도구는 `backend/mcp-server/index.js`를 단일 MCP 진입점으로 사용합니다.
...
| AI 도구 | 설정 파일 경로 |
|---|---|
| **Antigravity** (Gemini) | `.gemini/settings.json` (프로젝트) 또는 `~/.gemini/config/mcp_config.json` (글로벌) |
...
> [!IMPORTANT]
> npm/npx 레지스트리 패키지 기반 실행은 현재 지원되지 않으며 표준이 아닙니다. 레지스트리 버전은 최신 저장소 상태와 일치하지 않으므로, 반드시 로컬 저장소 루트에서 `node ./backend/bin/vibe-clinic.js` (Windows는 `node .\backend\bin\vibe-clinic.js`)를 사용하여 직접 실행해야 합니다.

```bash
# Windows PowerShell
node .\backend\bin\vibe-clinic.js init                        # .vibe-clinic/ 초기화 + MCP 자동 설정
node .\backend\bin\vibe-clinic.js run                         # 모든 진단 실행
node .\backend\bin\vibe-clinic.js run --json                  # JSON 출력 (CI/CD용)
node .\backend\bin\vibe-clinic.js dashboard                   # 웹 대시보드 열기
node .\backend\bin\vibe-clinic.js config get                  # BYOK 설정 확인
node .\backend\bin\vibe-clinic.js config set provider gemini  # AI 프로바이더 설정 (gemini 전용)
node .\backend\bin\vibe-clinic.js config set apiKey sk-...    # API 키 설정
node .\backend\bin\vibe-clinic.js config set model gemini-3.5-flash  # 모델명 설정
node .\backend\bin\vibe-clinic.js repair <diagId>             # 특정 진단 AI 자동 수리
node .\backend\bin\vibe-clinic.js repair --all                # 실패한 모든 진단 자동 수리
...
node ./backend/bin/vibe-clinic.js config set apiKey sk-...
...
> [!TIP]
> **대시보드 작동 검증 완료:** Windows PowerShell 환경에서 `node .\backend\bin\vibe-clinic.js dashboard` 실행 및 `http://localhost:7700`을 통한 API 호출 작동이 검증 완료되었습니다.
