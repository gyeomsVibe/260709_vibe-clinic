...
### ⚠️ 남은 과제: 팝업이 다른 창 뒤에 숨겨짐
- `Show(IntPtr.Zero)`로 부모 윈도우 핸들 없이 호출하면 포커스를 받지 못함
- **해결책**: `.ps1` 스크립트 파일로 분리하고, `Show()` 호출 전에 `[System.Windows.Forms.NativeWindow]`나 임시 폼을 생성하여 부모 핸들을 제공하거나, `SetForegroundWindow` API를 호출

### 📋 최종 구현 계획

1. **`.folder-picker.ps1` 스크립트 파일을 프로젝트에 포함** — 인라인 문자열이 아닌 독립 파일로 관리하여 인코딩 깨짐 완전 제거
2. **컴파일 제로 리플렉션 기법 적용** — 위에서 실증한 `CreateVistaDialog` → `SetOptions(FOS_PICKFOLDERS)` → `Show()` 흐름
3. **포그라운드 포커스 보장** — 임시 WinForms 폼을 부모 핸들로 전달하여 팝업이 반드시 최전면에 표시
4. **`dashboard.js`에서 `exec('powershell -STA -File .folder-picker.ps1')`로 호출** — Base64 인코딩 불필요
5. **폴더 선택 경로를 `stdout`으로 전달** — 기존 API 응답 구조 유지
