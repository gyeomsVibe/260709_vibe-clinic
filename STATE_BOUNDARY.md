...
| 4 | **VSIX 산출물 (구버전)** | `vscode-extension/vibe-diagnosis-vscode-1.1.0.vsix` 이하, `1.1.1.vsix` | 1.1.0 이하: **수정 전 버그 코드 포함** (dashboard 5초 종료 버그, publisher=Rejard). 1.1.1: 코드는 최신이나 **구계정(gyeoms-vibe) 메타데이터** | ⚠️ 배포 대상에서 제외. 현재 소스 기준 산출물은 `1.1.2.vsix` (publisher=gyeomsVibe). **1.1.2 실설치 검증**: Antigravity IDE(Code-OSS 1.107.0)에서 설치 성공(`gyeomsvibe.vibe-diagnosis-vscode@1.1.2`, 2026-07-10). 순정 VS Code는 이 머신에 미설치라 미검증, 확장 기능 동작 검증도 미실행(Phase V-2) |
