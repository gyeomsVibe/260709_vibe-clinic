...
| P2 | integrations/vscode-extension 락파일 drift | ✅ 완료 | lock을 1.1.2로 재생성 |
...
| VSIX 실설치 검증 | ✅ 부분 완료 (2026-07-10, Phase V) — **Antigravity IDE**(Code-OSS 1.107.0)에서 1.1.2 설치 성공. **순정 VS Code는 이 머신에 미설치(CommandNotFound)되어 검증 불가 확인** | 순정 VS Code 검증: VS Code 설치 후 `code --install-extension integrations/vscode-extension/vibe-diagnosis-vscode-1.1.4.vsix` |
...
| ⚠️ 확장 npx 폴백 결함 (Phase V-2에서 발견) | ✅ 완료 | `integrations/vscode-extension/src/extension.js` 내 npx fallback 명령을 `npx -y --package=vibe-diagnosis vibe-diag`로 수정하여 캐시 유무와 무관하게 정상 작동 확인. VSIX 버전을 1.1.3으로 올림. |
