  ...
  return (
    <article className={`metric-card metric-${tone}`}>
      ...
      <div className="metric-ring" style={style} aria-label={`${label}: ${numericValue === null ? '데이터 없음' : `${numericValue}%`}`}>
        <div className="metric-ring-core"><span className="metric-value">{numericValue === null ? '—' : `${numericValue}%`}</span><span className="metric-caption">{numericValue === null ? '데이터 없음' : '실시간 점수'}</span></div>
      </div>
      <p>{detail}</p>
    </article>
  )
